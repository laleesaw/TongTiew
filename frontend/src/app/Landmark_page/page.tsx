...
function useFindFromDatabase() {
    ...
    const fetchLandmark_Attraction_id = async ( search_number: number) => {
        try{
            const res = await fetch("http://localhost:8080/landmark",{
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({Lankmark_id: search_number}),
            })
  ...
  return { landmark, fetchLandmark_Attraction_id };
} 



function A_Card({src, name, rating, landmark}: A_card_type){
    if(!landmark){
        return(
            <div className = "card">
                Error
    ...
    return(
            <div className = "card">
                ...
                <h2 className = "detail">
                    <Detail_handler destination={landmark}></Detail_handler>
                </h2>
            </div>
    )
}
// function Display_card({landmark}: {landmark: Landmark | null}){
function Display_card(){
    const {landmark, fetchLandmark_Attraction_id} = useFindFromDatabase();
    ...
    useEffect(() => {
        fetchLandmark_Attraction_id(temp_id);
    }, [])


    let total_landmark: string[] = [];
    let total_src: string[] = [];
    let total_rating: string[] = []; 
    let total_detail: string[] = [];
    for( let i = 0; i < landmark.length; i++){
        total_landmark.push(landmark[i].name);
        total_src.push(landmark[i].img_path);
        total_rating.push(landmark[i].rating);
        total_detail.push(landmark[i].detail);
    }
    return(
        <div className = "display_card">
            {total_landmark.map((name, i) => (
                <A_Card 
                    key = {i}
                    src = {total_src[i]}
                    name = {total_landmark[i]}
...
export default function Landmark_page(){
    return(
        <div className = "page">
